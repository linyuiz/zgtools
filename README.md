...
&emsp;&emsp;运行zgtools GetTwoSyn进行syri的切分并行分析，展示基因组共线性（基因共线性建议JCVI）和染色体间的SV：INV（黄色）、TRANS（绿色）和 Dup（蓝色）。以T2T骏枣【[原文链接](https://doi.org/10.1093/hr/uhae071) 】的chr5和chr12染色体的移位为例，结果如下：
